...
For example you can enter a key values named sentiment and select the post call variable as sentiment to add the respective values.
...
3. From now on, whenever your agent reaches that specific step (for example, _Verify Caller Identity_), it will automatically execute the configured Zapier block.
